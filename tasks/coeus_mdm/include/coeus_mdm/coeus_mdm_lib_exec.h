...
void Del(const hipc::MemContext &mctx, u32 method, Task *task) override {
  switch (method) {
    ...
    case Method::kMdm_insert: {
      CHI_CLIENT->DelTask<Mdm_insertTask>(mctx, reinterpret_cast<Mdm_insertTask *>(task));
      break;
    }
  }
}
...
void CopyStart(u32 method, const Task *orig_task, Task *dup_task, bool deep) override {
  switch (method) {
    ...
    case Method::kMdm_insert: {
      chi::CALL_COPY_START(
        reinterpret_cast<const Mdm_insertTask*>(orig_task), 
...
void NewCopyStart(u32 method, const Task *orig_task, FullPtr<Task> &dup_task, bool deep) override {
  switch (method) {
    ...
    case Method::kMdm_insert: {
      chi::CALL_NEW_COPY_START(reinterpret_cast<const Mdm_insertTask*>(orig_task), dup_task, deep);
      break;
    }
  }
}
...
TaskPointer LoadStart(    u32 method, BinaryInputArchive<true> &ar) override {
  ...
  switch (method) {
    ...
    case Method::kMdm_insert: {
      task_ptr.ptr_ = CHI_CLIENT->NewEmptyTask<Mdm_insertTask>(
             HSHM_DEFAULT_MEM_CTX, task_ptr.shm_);
