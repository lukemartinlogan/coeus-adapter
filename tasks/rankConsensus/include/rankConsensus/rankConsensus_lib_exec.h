...
void Del(const hipc::MemContext &mctx, u32 method, Task *task) override {
  switch (method) {
    ...
    case Method::kGetRank: {
      CHI_CLIENT->DelTask<GetRankTask>(mctx,
                                       reinterpret_cast<GetRankTask *>(task));
      break;
    }
  }
}
...
void CopyStart(u32 method, const Task *orig_task, Task *dup_task,
  ...
  switch (method) {
    ...
    case Method::kGetRank: {
      chi::CALL_COPY_START(reinterpret_cast<const GetRankTask *>(orig_task),
...
void NewCopyStart(u32 method, const Task *orig_task, FullPtr<Task> &dup_task,
  ...
  switch (method) {
    ...
    case Method::kGetRank: {
      chi::CALL_NEW_COPY_START(reinterpret_cast<const GetRankTask *>(orig_task),
                               dup_task, deep);
      break;
    }
  }
...
TaskPointer LoadStart(u32 method, BinaryInputArchive<true> &ar) override {
  ...
  switch (method) {
    ...
    case Method::kGetRank: {
      task_ptr.ptr_ =
          CHI_CLIENT->NewEmptyTask<GetRankTask>(HSHM_MCTX, task_ptr.shm_);
