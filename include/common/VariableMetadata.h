...
// This structure is designed to capture metadata information about adios2
// variables.

enum class adiosOpType { get = 0, put = 1 };
...
struct metaInfo {
  ...
  metaInfo(const adios2::core::Variable<T> &variable, adiosOpType operationType,
           std::string blob, std::string bucket, std::string processor_name,
           int process_No) {
    ...
    time = std::to_string(localTime->tm_hour) + ":" +
  ...
  template <typename T>
  explicit metaInfo(const adios2::Variable<T> variable,
                    adiosOpType operationType, std::string blob,
  ...
  void serialize(Archive &ar) {
    ar(operation, time, name, sizeofVariable, shapeID, shape, start, count,
       constantShape, dataType, steps, stepStart, blockID, blob_name,
       bucket_name, processor, processNo);
  }
};

// Name, shape, start, Count, Constant Shape, Time, selectionSize,
// sizeofVariable, ShapeID, steps, stepstart, blockID, order.
std::ostream &operator<<(std::ostream &out, const metaInfo &data) {
  ...
  out << "," << data.time << "," << data.selectionSize << ","
      ...
      << data.stepStart << "," << data.blockID << "," << data.blob_name << ","
      << data.bucket_name << "," << data.processor << "," << data.processNo;
  return out;
}
