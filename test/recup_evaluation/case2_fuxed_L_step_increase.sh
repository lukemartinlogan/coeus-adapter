...
for i in ${!L_values[@]}; do
  ...
  jarvis pkg config adios2_gray_scott out_file=/mnt/${location}/hxu40/ofs-mount/case2/${nprocs}process/out1.bp ppn=20 nprocs=${nprocs} steps=$L L=512
  jarvis pkg config adios2_gray_scott_2 out_file=/mnt/${location}/hxu40/ofs-mount/case2/${nprocs}process/out2.bp ppn=20 nprocs=${nprocs} steps=$L L=512
